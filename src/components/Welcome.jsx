...
const Welcome = () => {
    ...
    const currentAccountTrimmed = (currentAccount) => {
        if (currentAccount.length > 10) {
            return currentAccount.substring(0, 10) + '...' + currentAccount.substring(currentAccount.length - 4, currentAccount.length);
        }
            ...
            <div className='flex mf:flex-row flex-col items-start justify-between md:p-20 py-12 px-4'>
                <div className='flex flex-1 justify-start flex-col mf:mr-10'>
                    ...
                    <div className='grid sm:grid-cols-3 grid-cols-2 w-full mt-10'>
                        <div className={`rounded-tl-2xl welcomeGrid`}>
                            Reliability
                        </div>
                        <div className={`rounded-th-2xl welcomeGrid`}>
                            Security
                        </div>
                        <div className={`rounded-tr-2xl welcomeGrid`}>
                            Ethereum
                        </div>
                        <div className={`rounded-bl-2xl welcomeGrid`}>
                            Web 3.0
                        </div>
                        <div className={`rounded-bh-2xl welcomeGrid`}>
                            Low fees
                        </div>
                        <div className={`rounded-br-2xl welcomeGrid`}>
                            Blockchain
                        </div>
                    </div>
                </div>
                        ...
                        <div className='flex justify-between flex-col w-full h-full'>
                            ...
                            <div>
                                <p className='text-white font-light text-sm'>
                                    Account address: {" "}
                                    {currentAccount ?
                                        currentAccountTrimmed(currentAccount) :
                                        "Not logged in yet"}
                                </p>
